(function () {
  ...
  async function refresh() {
    ...
    // UI kamu pakai __backendId untuk delete/update
    const mapped = (data || []).map((row) => ({
      ...row,
      __backendId: row.id,
      // jaga format items biar UI gak patah
  ...
  const supabaseDataSdk = {
    ...
    async create(invoiceData, file) {
      try {
        ...
        // insert dulu (biar dapet id), lalu upload PDF, lalu update row pdf_path
        const row = {
          id,
          user_id: userId,

          // mapping dari parsePDF kamu
        ...
        if (file) {
          ...
          if (up.error) {
            // rollback row kalau upload gagal
